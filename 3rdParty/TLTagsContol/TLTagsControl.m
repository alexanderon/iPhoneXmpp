...
- (void)deleteTagButton:(UIButton *)sender {
    ...
    NSInteger index = [tagSubviews_ indexOfObject:view];
    NSDictionary *dict =[_tags objectAtIndex:index];
    NSInteger *indexOfObject =[[dict valueForKey:@"index"]integerValue];
    [_tags removeObjectAtIndex:index];
    [tapDelegate tagsControl:self deletedAtIndex:indexOfObject];
        [self reloadTagSubviews];
}
